...
this.leftBtn.css({background:this.mainColorObj.rgba(.6)});this.rightBtn.css({background:this.mainColorObj.rgba(.6)})},initDefaultDom:function(){this.closeUrl=form_icons.close?"data:image/png;base64,"+form_icons.close:uiBaseURL+"close.png";this.close=$("<img alt='close' width='auto' height='auto' class='close' src='"+this.closeUrl+"' />");this.close.click(this.hide.bind(this));this.close.changeButtonColor(this.fontColor);this.leftBtn=$("<div class='leftBtn btn'><img src='"+this.leftButtonUrl+"'/></div>");
...
e.getClassName()&&global.WeChatShareImg.setDirection(TextBubble.LEFT),"TelephoneNumberButton"==e.getClassName()&&global.PcTelephoneNumberFrame.setDirection(TextBubble.LEFT),e.addTo(this.extended),e.hint=!1)}b<this.buttonCount&&(this.btnMore.addTo(this.container.body),this.btnMore.setPosition(a,this.buttonTop))},addPageButtons:function(a){function b(p){p.addTo(f);p.setPosition(g,l);g+=h}function c(p){p.addTo(f);p.setPosition(g,14);g+=p.width()+5}function d(){g+=n;0<e?(m.css({left:g+"px",top:k+"px",
width:e+"px",display:"block"}),m.find("input").css({width:e+"px"}),g+=e):m.css({display:"none"});g+=n}var e=this.edtNumber.width,f=this.container.body,g=a,h=this.buttonWidth,l=this.buttonTop,m=this.edtNumber.body,k=this.edtNumber.top,n=this.edtNumber.space;rightToLeft?(c(this.rightBevel),bookConfig.enablePageBack&&b(this.btnForward),b(this.btnLast),b(this.btnNext),d(),b(this.btnPrevious),b(this.btnFirst),bookConfig.enablePageBack&&b(this.btnBackward),c(this.leftBevel)):(c(this.leftBevel),bookConfig.enablePageBack&&
